...
void string_render(
  char* str_array, SDL_Renderer* renderer, TTF_Font* font, SDL_Color color, int x, int y);
...
void render_snake(Game* game) {
  ...
  SDL_FRect head_rect = {game->snake.head.x, game->snake.head.y, grid_size, grid_size};
  SDL_RenderFillRect(game->renderer, &head_rect);
  for (int i = 0; i < game->snake.length; i++) {
    SDL_FRect body_rect = {game->snake.body[i].x, game->snake.body[i].y, grid_size, grid_size};
...
void render_text(Game* game) {
  ...
  char timer[32];
  snprintf(timer, sizeof(timer), "Timer: %.1f s", (game->timer - game->last_time) / 1000.f);
  string_render(timer, game->renderer, game->font, white, 570, 10);

  char score_label[32];
  snprintf(score_label, sizeof(score_label), "Score: %04d", game->score);
  string_render(score_label, game->renderer, game->font, white, 10, 10);

  char length_label[32];
  snprintf(length_label, sizeof(length_label), "Snake length: %02d", game->snake.length);
  string_render(length_label, game->renderer, game->font, white, 250, 10);
}
