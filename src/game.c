...
bool game_init(Game* game) {
  ...
  if (!(game->window = SDL_CreateWindow("snake", screen_width, screen_height, 0))) {
  ...
  if (!(game->font = TTF_OpenFont("/Users/pavel/github/snake/font/font.ttf", 32))) {
  ...
  Snake snake
    = {.head = {screen_width / 2, screen_height / 2}, .direction = Right, .length = initial_len};
